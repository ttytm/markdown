...
fn test_attribute_transformer() ! {
	transformer := fn (parent markdown.ParentType, name string, value string) string {
		if parent is markdown.SpanKind && parent == .a {
			if name == 'href' && value == '.' {
	...
	mut renderer := markdown.HtmlRenderer{
		transformer: &markdown.AttrTransformerFn(transformer)
	}
...
fn (f &TestCodeFormatter) transform_content(parent markdown.ParentType, text string) string {
	if text.trim_space().len != 0 && parent is markdown.BlockKind && parent == .code {
